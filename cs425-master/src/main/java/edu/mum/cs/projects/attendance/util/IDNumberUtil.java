...
public class IDNumberUtil {
	...
	public static String convertToStudentId(long id){
		...
		// initialize the id to always be used to extract, the parts 1,2,3, the finalString, 
		// a copy of the id
		long lastFourDigits,middleTwoDigits,firstThreeDigits;
		long idCopy = id;
		String answer ="";
		// extract first four digits -> part1, update the idCopy
		lastFourDigits = idCopy % 10000;
		idCopy = idCopy / 10000;
		// extract next two digits -> part2 , update the idCopy
		middleTwoDigits = idCopy % 100;
		idCopy = idCopy / 100;
		// extract last three digits -> part3, use the idCopy directly
	...
	public static String convertToStudentId(String id){
		...
		// check if its not already in the required format
		if(!id.matches("\\d+")){
			return id;
		}
		// initialize the id to always be used to extract, the parts 1,2,3, the finalString, 
